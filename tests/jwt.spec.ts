...
test("a token that is not one says which part it fell down on", async ({ page }) => {
  ...
  await expect(page.getByText("A JWT is three parts separated by dots; this has 2")).toBeVisible();

  await tokenBox(page).fill("a.b.c.d.e");
  await expect(page.getByText("That is an encrypted JWE; this page reads signed tokens")).toBeVisible();
...
test("an empty box opens the builder on a token of its own", async ({ page }) => {
  await openJwt(page);
  await switchTo(page, "Encode");
  ...
  await expect(claimName(page, 1)).toHaveValue("sub");
  await expect(claimValue(page, 1)).toHaveValue(
    /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/,
  );
  await expect(claimName(page, 2)).toHaveValue("iat");
  await expect(claimName(page, 3)).toHaveValue("exp");

  const issued = Number(await claimValue(page, 2).inputValue());
  expect(Number(await claimValue(page, 3).inputValue()) - issued).toBe(3600);
...
test("what the builder made goes back to Decode, secret and all", async ({ page }) => {
  ...
  await claimName(page, 4).fill("role");
  await claimValue(page, 4).fill("admin");
  ...
  await switchTo(page, "Encode");
  await expect(claimName(page, 4)).toHaveValue("role");
  await expect(tokenBox(page)).toHaveValue(built);
});
...
test("a row can be taken back out", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Remove claim 3" }).click();
  await expect(claimName(page, 3)).toHaveCount(0);
  await switchTo(page, "Decode");
