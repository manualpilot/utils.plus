...
export function runMessage(run: Run, scope: Scope | null): string {
  ...
  if (outOfMemory(run)) return "The engine ran out of memory, so the variables could not be read.";
...
export function sessionMessage(run: Run, scope: Scope | null, session: Session): string {
  ...
  if (session.entries.length > 0) {
    return `The engine ${
      outOfMemory(run) ? "ran out of memory" : "was stopped"
    }, and the session's names went with it.`;
  }
...
export const OUT_OF_MEMORY_NOTE = "Out of memory, and the session's names went with the engine.";

function outOfMemory(run: Run): boolean {
  return run.state === "failed" && run.message === OUT_OF_MEMORY;
}
