...
async function answer(request: Request) {
  try {
    ...
    send({ kind: "done", id: request.id, scope, seconds, exhausted: started.exhausted || undefined });
  } catch (error) {
    flush();
...
export type Message =
  ...
  | { kind: "done"; id: number; scope?: Scope | null; seconds?: number; failed?: string; exhausted?: boolean };
